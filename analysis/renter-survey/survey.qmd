...
prop_Q12 %>%
  errorbar_barchart("Q12",
    "Can you keep comfortably warm in hyour house during winter?")
```
...
prop_Q13 %>%
  errorbar_barchart("Q13",
    "Can you keep comfortably cool in hyour house during summer?")
```
...
list(winterwarm = prop_Q12, summercool = prop_Q13) %>%
  ...
  mutate(
    ntotal = min(ntotal),
    signif = if_else(plow[1] <= phigh[2] & plow[2] <= phigh[1], "❌", "✅")) %>%
  ungroup() %>%
  ...
  mutate(
    ...
    season = case_match(question,
      "winterwarm" ~ "Winter",
      "summercool" ~ "Summer")) %>%
  select(zone, season, signif, ntotal, n, p, moe, nlow, plow, nhigh, phigh) %>%
